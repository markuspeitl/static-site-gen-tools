...
import { defaultMarkdownDataExtractor } from "./md-runner";
import * as nunjucks from 'nunjucks';

export const defaultNjkDocumentCompiler: DocumentCompiler = {
...
export const defaultNjkDataExtractor: DataExtractor = {
    extractData: async (fileContent: string, config?: any) => {

        let dataExtractor = config.dataExtractors[ 'md' ];
        if (!dataExtractor) {
            dataExtractor = defaultMarkdownDataExtractor;
        }
